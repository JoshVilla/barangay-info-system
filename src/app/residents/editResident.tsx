...
import { EDIT_RESIDENT } from "@/graphql/mutations/residents/editResident";
...
const EditResident = ({ refetch, records }: Props) => {
  const {
    id,
    firstname,
    middlename,
  ...
  const [editResident, { loading, error }] = useMutation(EDIT_RESIDENT);
  ...
  const onSubmit = async (values: ResidentForm) => {
    try {
      ...
      setOpenDialog(false);
      refetch();
      toast(data.editResident.message);
    } catch (error) {
      console.log(error);
    }
  };
